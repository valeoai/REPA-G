...
class SiT(nn.Module):
    ...
    def inference_feats(self, x, t, y, use_projector=True):
        x = self.x_embedder(x) + self.pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
        N, T, D = x.shape
        ...
        for i,block in enumerate(self.blocks[:self.encoder_depth]):
            ...
            if (i + 1) == self.encoder_depth:
                if use_projector:
                    ...
                    zs_tilde = [x]
        with torch.no_grad():
            for block in self.blocks[self.encoder_depth:]:
                x = block(x, c)                      # (N, T, D)
            x = self.final_layer(x, c)               # (N, T, patch_size ** 2 * out_channels)
            x = self.unpatchify(x)#,shape=shape)                   # (N, out_channels, H, W)
        return x , zs_tilde
    
    @torch.no_grad()
    def forward_feats(self, x, t, y, depth, use_projector=False):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if (i + 1) == depth:
                if use_projector:
                    D = x.shape[-1]
                    zs_tilde = [projector(x.reshape(-1, D)).reshape(x.shape[0], x.shape[1], -1) for projector in self.projectors]
                    return zs_tilde
                return x
