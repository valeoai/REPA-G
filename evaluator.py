...
INCEPTION_V3_PATH = os.path.join(os.environ["TORCH_HOME"], "inception_v3", "inception-2015-12-05.tgz")
...
def main(argv=None):
    ...
    print("Sample batch:", args.sample_batch)
    ...
    if args.output_file:
        ...
        with open(args.output_file, 'w') as f:
            f.write(f"Sample folder: {args.sample_batch}\n")
    ...
    # Compute and write Inception Score
    try:
        ...
        if args.output_file:
            with open(args.output_file, 'a') as f:
                f.write(f"Inception Score: ERROR - {e}\n")
    
    # Compute and write FID
    try:
        ...
        if args.output_file:
            with open(args.output_file, 'a') as f:
                f.write(f"FID: ERROR - {e}\n")
    
    # Compute and write sFID
    try:
        ...
        if args.output_file:
            with open(args.output_file, 'a') as f:
                f.write(f"sFID: ERROR - {e}\n")
    
    # Compute and write Precision/Recall
    try:
        ...
        if args.output_file:
            with open(args.output_file, 'a') as f:
                f.write(f"Precision/Recall: ERROR - {e}\n")
...
def _download_inception_model():
    ...
    with requests.get(INCEPTION_V3_URL, stream=True) as r:
        r.raise_for_status()
        tmp_path = INCEPTION_V3_PATH + ".tmp" + str(os.getpid())
        ...
        try:
            ...
            # Another process already downloaded it
            os.remove(tmp_path)
        except FileNotFoundError:
            # Tmp file was removed by another process, but final file should exist
